...
void OpenGlSlider:: redoImage(bool skip_image) {
  ...
  if (isModulationKnob()) {
    ...
    if (t > 0.0f) {
      slider_quad_->setShaderValue(0, davis::utils::interpolate(davis::kPi, -davis::kPi, t));
    ...
    slider_quad_->setShaderValue(0, davis::utils::interpolate(-arc, arc, t));
    slider_quad_->setColor(selected_color_);
    slider_quad_->setAltColor(unselected_color_);
    slider_quad_->setThumbColor(thumb_color_);
    slider_quad_->setStartPos(bipolar_ ? 0.0f : -davis::kPi);
...
//
//
//    setRotaryParameters(2.0f * davis::kPi - kRotaryAngle, 2.0f * davis::kPi + kRotaryAngle, true);
//
//
//
//
...
//  if (string_lookup_) {
//    int lookup = davis::utils::iclamp(value, 0, getMaximum());
...
//  davis::ValueDetails* details = getDisplayDetails();

  double adjusted_value = value;
  return value;
//  switch (details->value_scale) {
//    case davis::ValueDetails::kQuadratic:
//      adjusted_value *= adjusted_value;
//      break;
//    case davis::ValueDetails::kCubic:
//      adjusted_value *= adjusted_value * adjusted_value;
//      break;
//    case davis::ValueDetails::kQuartic:
//      adjusted_value *= adjusted_value;
//      adjusted_value *= adjusted_value;
//      break;
//    case davis::ValueDetails::kExponential:
//      adjusted_value = powf(display_exponential_base_, adjusted_value);
//      break;
//    case davis::ValueDetails::kSquareRoot:
...
//  if (details_.value_scale == davis::ValueDetails::kIndexed)
...
//    for (davis::ModulationConnection* connection : connections) {
